...
# Gestão do Salário na Sidebar
st.sidebar.subheader("💵 Configurar Salário")
sal_atual = banco.buscar_salario(st.session_state.user)
st.sidebar.write(f"Atual: **{f_moeda(sal_atual)}**")

n_sal = st.sidebar.number_input("Novo Salário:", min_value=0.0, value=float(sal_atual), step=100.0, format="%.2f")
...
if escolha == "💸 Lançar Gasto":
    ...
    if not df.empty:
        ...
        df['item'] = df['id'].astype(str) + " | " + df['descricao'] + " | " + df['valor'].apply(f_moeda)
        it_del = st.selectbox("Selecione:", df['item'].tolist())
